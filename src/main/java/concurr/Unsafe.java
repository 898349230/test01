...
public final class Unsafe {
	 ...
	 * field or array element within the given object <code>o</code> at the given
	 * offset, or (if <code>o</code> is null) from the memory address whose
	 ...
	 * by <code>o</code> is of a class compatible with that field's class.
	 *
	 * <li>The offset and object reference <code>o</code> (either null or non-null)
	 * were both obtained via {@link #staticFieldOffset} and
	 * {@link #staticFieldBase} (respectively) from the reflective {@link Field}
	 * representation of some Java field.
	 *
	 * <li>The object referred to by <code>o</code> is an array, and the offset is
	 ...
	 * @param offset
	 *            indication of where the variable resides in a Java heap object, if
	 *            any, else a memory address locating the variable statically
	 ...
	 * <p>
	 * Unless the reference <code>x</code> being stored is either null or matches
	 * the field type, the results are undefined. If the reference <code>o</code> is
	...
	/**
	 * This method, like all others with 32-bit offsets, was native in a previous
	 * release but is now a wrapper which simply casts the offset to a long value.
	 * It provides backward compatibility with bytecodes compiled against 1.4.
	 * 
	 ...
	 * This method is implemented as follows: <blockquote>
	 * 
	 * <pre>
	 ...
	 * </blockquote>
	 * 
	 * @deprecated As of 1.4.1, use {@link #staticFieldBase(Field)} to obtain the
	 *             base pertaining to a specific {@link Field}. This method works
	 *             only for JVMs which store all statics for a given class in one
	 *             place.
	 */
	@Deprecated
	public Object staticFieldBase(Class c) {
	 ...
	 * </ul>
	 * 
	 * @params hostClass context for linkage, access control, protection domain, and
	 *         class loader
	 * @params data bytes of a class file
	 * @params cpPatches where non-null entries exist, they replace corresponding CP
	 *         entries in data
	 */
	...
	/**
	 * Tries to lock the object. Returns true or false to indicate whether the lock
	 * succeeded. If it did, the object must be unlocked via {@link #monitorExit}.
	 */
	 ...
	 * cause the subsequent call to <tt>park</tt> not to block. Note: this operation
	 ...
	 * is in the Unsafe class only because <tt>unpark</tt> is, so it would be
